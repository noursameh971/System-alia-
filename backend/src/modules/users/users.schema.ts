...
export const createUserSchema = z
  .object({
    ...
    role: z.enum(["admin", "warehouse_staff", "finance"]),
    // Required for warehouse_staff and finance (their one workspace),
    // forbidden for admin (never scoped to a single brand) — see the refine
    // below.
    brandId: z.string().uuid().optional(),
  })
  .refine((data) => (data.role === "admin" ? !data.brandId : !!data.brandId), {
    message: "brandId is required unless role is 'admin', and must be omitted for 'admin'",
    path: ["brandId"],
  });
...
export const updateUserSchema = z
  .object({
    ...
    role: z.enum(["admin", "warehouse_staff", "finance"]).optional(),
