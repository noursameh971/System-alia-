...
const PRINT_ROOT_ID = "qr-label-print-root";
 ...
 *
 * QR labels used to print via a `.print-area` element hidden in place with
 ...
 * (each meant to land on its own physical 50mm x 25mm page via
 * `break-after-page`) couldn't paginate at all and rendered cropped and at
 * the wrong scale. Portaling to <body> sidesteps both: nothing precedes it,
 * so it flows and paginates like a normal document (see globals.css's
 * `#qr-label-print-root` rules).
 */
export function QrLabelPrintPortal({ children }: { children: ReactNode }) {
