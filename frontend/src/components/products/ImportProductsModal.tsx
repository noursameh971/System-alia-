...
import { importProductsWorkbook } from "@/lib/products";
import type { ImportProductsResult } from "@/lib/types";
...
export function ImportProductsModal({ open, onOpenChange, brandId, onSuccess }: ImportProductsModalProps) {
  ...
  async function handleImport() {
    ...
    setSubmitting(true);
    try {
      const outcome = await importProductsWorkbook(brandId, file);
      setResult(outcome);
      if (outcome.created + outcome.updated > 0) onSuccess();
      if (outcome.errors.length === 0) {
        toast.success(`Imported ${outcome.created} new and updated ${outcome.updated} existing variant${outcome.updated === 1 ? "" : "s"}`);
      } else {
    ...
    >
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload an .xlsx sheet to bulk-update existing prices/stock (matched by SKU) or create new products
            (matched by Product Name + Color + Size). Exported files re-import cleanly.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="flex flex-col gap-2">
              <div className="grid grid-cols-4 gap-2 text-center">
                ...
                <div className="rounded-lg bg-blue-50 px-2 py-2 dark:bg-blue-950">
                  <p className="text-base font-semibold text-blue-700 dark:text-blue-300">{result.updated}</p>
